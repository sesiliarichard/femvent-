...
export default function SignupPage() {
  ...
  const handleOrgNameChange = (text: string) => {
    setOrgName(text);
    if (text && text.length < 2) {
      setOrgNameError('Organization name must be at least 2 characters');
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !orgName || !email || !password || !confirmPassword) {
    ...
    if (!isEmailValid || !isNameValid || !isOrgNameValid || !isPasswordValid || !isConfirmPasswordValid) {
        ...
        <div className="border border-gray-200/30 bg-white/60 backdrop-blur-xl shadow-2xl shadow-black/20 rounded-lg p-8">
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="relative">
              ...
              />
              {orgNameError && <p className="mt-1 text-xs text-red-600">{orgNameError}</p>}
            </div>
