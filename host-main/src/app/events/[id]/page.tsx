            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="w-8 h-8 flex items-center justify-center text-gray-500">{getEventTypeIcon(event.type)}</span>
                    <span className="px-4 py-2 rounded-full text-sm font-bold text-white bg-primary-600">
                      {event.type}
                    </span>
                    {event.multi_day && (
                      <span className="px-4 py-2 rounded-full text-sm font-bold text-secondary-700 bg-secondary-50 border border-secondary-200">
                        Multi-Day Event
                      </span>
                    )}
                  </div>
                ...
                {isEventOwner && (
                  <div className="flex gap-3 flex-wrap">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-6 py-3 rounded-xl font-bold text-sm hover:border-primary-300 transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-xl font-bold text-sm transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isEventOwner && (
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold text-sm hover:border-primary-300 hover:bg-primary-50 hover:text-primary-700 transition-colors"
                  >
                      ...
                      {event.speakers.map((speaker: any, idx: number) => (
                        <div key={idx} className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                          <div className="flex items-start gap-4">
                            <div className="w-12 h-12 bg-primary-600 rounded-xl flex items-center justify-center text-white text-lg font-extrabold flex-shrink-0">
                              {speaker.name.charAt(0)}
                            </div>
                            <div className="flex-1">
                              <h3 className="text-base font-extrabold text-gray-900 mb-1">{speaker.name}</h3>
                              <p className="text-sm font-bold text-primary-600 mb-2">{speaker.title}</p>
                              {speaker.company && <p className="text-sm text-gray-600 mb-2">{speaker.company}</p>}
...
{event.agenda && event.agenda.length > 0 && (
                  <div className="bg-white rounded-2xl border border-gray-200 p-8">
                    ...
                    <div className="space-y-4">
                      {event.agenda.map((item: any, idx: number) => {
                        ...
                        return (
                          <div key={idx} className="flex gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-200">
                            <div className="flex-shrink-0">
                              <div className="w-20 h-20 bg-primary-600 rounded-xl flex flex-col items-center justify-center text-white">
               ...
               <div className="space-y-6">
                <div className="bg-white rounded-2xl border border-gray-200 p-8 sticky top-8">
                  <h2 className="text-lg font-extrabold text-gray-900 mb-6">Event Details</h2>

                  <div className="space-y-5">
                    <div className="pb-5 border-b border-gray-100">
                      <div className="flex items-start gap-4">
                        <div className="w-11 h-11 bg-primary-600 rounded-xl flex items-center justify-center text-white flex-shrink-0"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg></div>
                        <div>
                          <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Start Date</p>
                          <p className="text-sm font-bold text-gray-900">{formatDate(event.start_at)}</p>
                        </div>
                      </div>
                      {event.multi_day && event.end_at && (
                        <div className="flex items-start gap-4 mt-4">
                          <div className="w-11 h-11 bg-secondary-500 rounded-xl flex items-center justify-center text-white flex-shrink-0"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3v18h18M6 3v12m6-12v8m6-8v4" /></svg></div>
                          <div>
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">End Date</p>
                            <p className="text-sm font-bold text-gray-900">{formatDate(event.end_at)}</p>
                          </div>
                        </div>
                      )}
                    </div>
